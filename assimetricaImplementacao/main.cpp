...
** Hospedeiro inicial
 	OK --> Arvores Minimas (calculadas de maneira simétrica, mesmo que a instância seja assimétrica)
...
void constroiPlasmideosFromTree(Grafo *g, vector <Pasmideo> &plasmideosSimples, vector< Informacao > trees){
	for (int i=0; i<quantPlasFromTree; i++){
		...
		while (fila_de_vertices.size()>0 && conttttvertices<7){ // TAMANHO
			...
			if (orig!=destino){
				for (int jj = 0; jj<tree.caminho.size(); jj++){
					...
					// } // deixa comentado, pois é assimétrica
				}
			} else break;
		}
		...
		while (dest != origem){
			...
			sol.push_back(g->getArestas(g->isAdjacente(antecessor[dest], dest))); //assimetrica
			dest = antecessor[dest];
			//cout<<"aqui3"<<endl;
		}
		vector<Aresta*> sol2; // assimétrica
		for (int i=sol.size()-1; i>=0; i--){
			sol2.push_back(sol[i]);
		}
		if (sol.size()>0){
			plasmideosSimples.push_back((Pasmideo){sol2});
...
bool manipula(Grafo *g, Pasmideo plasm, Informacao cromossomo, Informacao &resultado){
	...
	do{
		...
		for (int i=0; i<cromossomo.caminho.size(); i++){ // para cada gene do cromossomo, verifica-se se ele pode ser inserido no fim ou no inicio
			if (visitado[cromossomo.caminho[i]->getOrigem()]!=2 && visitado[cromossomo.caminho[i]->getDestino()]!=2 && visitado[cromossomo.caminho[i]->getOrigem()]!=visitado[cromossomo.caminho[i]->getDestino()]){
				if (cromossomo.caminho[i]->getOrigem()==fim){ //cromossomo.caminho[i]->getDestino()==fim || ASSIMETRICAS
					...
					fim = cromossomo.caminho[i]->getDestino();//cromossomo.caminho[i]->getOrigem()!=fim?cromossomo.caminho[i]->getOrigem():cromossomo.caminho[i]->getDestino();
					size++;
					isAdded = true;
					//cout<<cromossomo.caminho[i]->getOrigem()<<", "<<cromossomo.caminho[i]->getDestino()<<" ";
				
					//cout<<"AQUI1"<<endl;
					//cout<<visitado[0]<<" "<<visitado[1]<<" "<<visitado[2]<<" "<<visitado[3]<<" "<<visitado[4]<<visitado[5]<<endl;
				} else if (cromossomo.caminho[i]->getDestino()==init){ // cromossomo.caminho[i]->getOrigem()==init || idem
					it =  resultado.caminho.begin();
					resultado.caminho.insert(it, cromossomo.caminho[i]);
					resultado.custo +=cromossomo.caminho[i]->getPeso();
					visitado[cromossomo.caminho[i]->getOrigem()]+=1;
					visitado[cromossomo.caminho[i]->getDestino()]+=1;
					init = cromossomo.caminho[i]->getOrigem();//cromossomo.caminho[i]->getOrigem()!=init?cromossomo.caminho[i]->getOrigem():cromossomo.caminho[i]->getDestino();
					//cout<<cromossomo.caminho[i]->getOrigem()<<", "<<cromossomo.caminho[i]->getDestino()<<" ";
	...
	if (size==cromossomo.caminho.size()-1){
		fim = resultado.caminho[size - 1]->getDestino();//visitado[resultado.caminho[size - 1]->getDestino()]==1?resultado.caminho[size - 1]->getDestino():resultado.caminho[size - 1]->getOrigem();
		init = resultado.caminho[0]->getOrigem(); // visitado[resultado.caminho[0]->getDestino()]==1?resultado.caminho[0]->getDestino():resultado.caminho[0]->getOrigem();
		if (g->isAdjacente(fim,init)!=-1){ // pra fechar o ciclo
		...
		do{
			...
			for (int iiii=0; iiii<g->getQuantVertices(); iiii++){
				if (visitado[iiii]==0){
					...
					if (g->isAdjacente(fim, iiii)!=-1){
						if (g->getArestas(g->isAdjacente(fim,iiii))->getPeso()<min){
							id_min = g->isAdjacente(fim,iiii);
							min = g->getArestas(id_min)->getPeso();
							no_init = false;
						}
					}
					if (id_min!=-1){
						...
						//cout<<g->getArestas(id_min)->getOrigem()<<", "<<g->getArestas(id_min)->getDestino()<<" ";
...
int main(){
	...
	// }
	// Informacao resultado;
	// bool isValido = manipula(&my_grafo, plasmideosSimples[2], endossibiontes[0].first, resultado);
	// if (isValido){
	// 	for (int j=0; j<resultado.caminho.size(); j++){
